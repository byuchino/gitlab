def regex_filter_list_handle_empty(input_list=None, regex=None, action=None, **kwargs):
    """
    Filter values in a list using a regex and either keep or drop values that match, depending on the action parameter.
    
    Args:
        input_list (CEF type: *): The list of items to filter using regex
    ...
    # handle case where all items on input list were dropped
    if not outputs:
