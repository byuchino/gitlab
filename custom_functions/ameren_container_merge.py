def ameren_container_merge(target_container=None, container_list=None, workbook=None, close_containers=None, **kwargs):
    """
    An alternative to the add-to-case API call.  This function will copy all artifacts, automation, notes and commands over from every container within the container_list into the target_container.  The target_container will be upgraded to a case.  The notes will be copied over with references to the child containers from where they came.  A note will be left in the child containers with a link to the target container.  The child containers will be marked as evidence within the target container.  Any notes left as a consequence of the merge process will be skipped in subsequent merges.
    ...
    for child_container_id in container_list:
        ...
        data_row = "{0} | [{1}](/mission/{0}) |".format(container['id'], container['name'])
    ...
    for child_container_id,child_container_name in zip(child_container_list,child_container_name_list):
        format_list.append("| {0} | [{1}](/mission/{0}) |\n".format(child_container_id, child_container_name))
